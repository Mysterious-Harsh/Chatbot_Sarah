...
class sarah():
    ...
    def __init__(self):
        ...
        f = Frame(self.root, height =40, width = self.wow, bg = "#33CCFF",highlightthickness=0,relief=FLAT)
        f.pack(fill=BOTH,expand=1)
        f.propagate(0)
        logo = PhotoImage(file=r"images/2.png",)
        w1 = Label(f, image=logo,relief=FLAT,bg="#33ccFF").pack(side="left")

        self.t = Label(f, text = 'Dana', height = 4 ,font = ('nunito', 20, 'bold'), fg = "black",bg="#33ccFF")
        self.t.pack(fill=X)
        self.c = Canvas(self.root,bg="white",height=self.how-150,width=self.wow,relief=FLAT,highlightthickness=0)
        ...
        self.c1 = Canvas(self.root,bg="white",height=100,width=self.wow,relief=FLAT,highlightthickness=0)
        self.c1.pack(fill=BOTH,expand=1)
       
        self.arect = self.c1.create_rectangle(100,5,self.wow-100,51,width=0,fill="#0099FF")
        self.t=Entry(self.c1,width=100,font=('nunito',14,'bold'),bg = "white",highlightthickness=0,relief=FLAT)
        ...
        name=self.c1.create_text(100,60,text='Creation by Harsh Patel',font=('nunito',14,"bold"),fill="black",anchor=W)

        photo = PhotoImage(file = r"images/microphone.png") 
        photo = photo.subsample(7,7) 
        

        self.b=Button(self.c1, height=35,width=35,image = photo,command=lambda:self.speech(),relief=FLAT,bg='#0099FF',highlightthickness=0)
    ...
    def resize(self,e):
        ...
        for inp,ans in self.conver:
            ...
            p=(self.wow-100)-(y*12)
            uinp = self.c.create_text(p,nl,text=inp,font=('helvetica',14,"bold"),fill="white",anchor=NW,width=w)
            x,y,x1,y1 = self.c.bbox(uinp)
            self.c.delete(uinp)
            uinp = self.c.create_text((self.wow-130)-(x1-x),nl,text=inp,font=('nunito',14,"bold"),fill="white",anchor=NW,width=w,activefill='black')
            x,y,x1,y1 = self.c.bbox(uinp)
            ...
            text = self.c.create_text(130,nl,text=ans,font=('nunito',14,"bold"),fill="black",anchor=NW,width=w,activefill='White')
    ...
    def speech(self):
        ...
        with sr.Microphone() as source:
            ...
            say = re.choice(["Say something","Tell me","Hello sir","how can I help you",'What can I do for you'])
            self.engine.say(say) 
